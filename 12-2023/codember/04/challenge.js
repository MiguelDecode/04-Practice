...
arrFiles.forEach((file) => {
  ...
  for (const character of chain) {
    const appeareanceLetter = chain.split(character).length - 1;
    if (appeareanceLetter === 1) sum += character;
  }

  if (unchecksum === sum) realFiles.push(sum);
});

console.log(realFiles);
console.log(`submit ${realFiles[resultPos]}`);

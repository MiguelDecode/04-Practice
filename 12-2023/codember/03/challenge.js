...
SECURITY_POLICIES_RULES.split("\n").forEach((securityPolicy) => {
  const [entireRule, key] = securityPolicy.split(":");
  const formatKey = key.trim();
  const [appearences, letter] = entireRule.split(" ");
  const [min, max] = appearences.split("-");

  const lettersInKey = formatKey.split(letter).length - 1;

  if (!(lettersInKey >= Number(min) && lettersInKey <= Number(max))) {
    invalidKeys.push(formatKey);
  }
});

console.log(`submit ${invalidKeys[SELECTED_KEY_POS]}`);
